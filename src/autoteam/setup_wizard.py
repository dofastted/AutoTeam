...
REQUIRED_CONFIGS = [
    ...
    ("PROXY_NODE_ENABLED", "启用代理节点接口（true/false）", "false", True),
    ("PROXY_NODE_PROVIDER", "代理节点提供者（none/webshare）", "none", True),
    ("PROXY_NODE_API_KEY", "代理节点 API Key", "", True),
    ("PROXY_NODE_BASE_URL", "代理节点 API 地址", "https://proxy.webshare.io/api/v2", True),
    ("PROXY_NODE_PROTOCOL", "节点写入协议（http/socks5/socks5h）", "http", True),
    ("PROXY_NODE_AUTO_REFRESH", "自动化任务前刷新节点（true/false）", "false", True),
    ("PROXY_NODE_REFRESH_BEFORE_TASK", "每个后台任务开始前轮询新节点（true/false）", "true", True),
    ("PROXY_NODE_POLL_INTERVAL_SECONDS", "节点轮询间隔秒数", "5", True),
    ("PROXY_NODE_POLL_TIMEOUT_SECONDS", "节点轮询超时秒数", "120", True),
    ("PROXY_NODE_COUNTRY", "节点刷新锁定国家（可选，如 US）", "", True),
    ("PROXY_NODE_APPLY_TO_OUTBOUND_POOL", "刷新后写入全局出口代理池（true/false）", "true", True),
    ("API_KEY", "API 鉴权密钥（回车自动生成）", "", False),
