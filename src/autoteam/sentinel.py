...
移植自 X:/project/Gpt-Agreement-Payment/CTF-reg/sentinel.py
（基于 https://github.com/leetanshaj/openai-sentinel）。
此文件不依赖第三方 pybase64，改用标准库 base64 实现兼容。

流程: PoW 计算 → 请求 /sentinel/req → 解析 Turnstile → 组装 Token

OpenAI 反欺诈系统对所有 OTP 相关 endpoint
(/email-otp/send, /email-otp/resend, /email-otp/validate,
 /passwordless/send-otp, /password/verify, /authorize/continue 等)
要求请求头 `openai-sentinel-token` 携带本函数生成的 token。
没有该 header 时 OpenAI 静默丢弃请求 / 不下发 OTP，
表现为客户端 OTP 等待超时（邮箱真的收不到信）。
"""
...
def generate_pow_token(user_agent: str = DEFAULT_UA) -> str:
    ...
    if found:
        logger.debug("Sentinel PoW 计算成功，token 长度 %d", len(token))
    else:
        logger.warning("Sentinel PoW 未找到解，使用 fallback")
    return token
...
def get_sentinel_token(
    ...
    1. PoW 计算
    2. 请求 sentinel.openai.com /sentinel/req 获取 Turnstile challenge
    3. 组装完整 token
    """
    device_id = (device_id or "").strip()
    if not device_id:
        raise ValueError("get_sentinel_token requires non-empty device_id")
    ...
    try:
        ...
        logger.warning("Sentinel req 网络异常: %s, 使用 PoW-only token", exc)
        return json.dumps({
            "p": pow_token, "t": "", "c": "",
    ...
    if resp.status_code != 200:
        logger.warning(
            "Sentinel req 返回 HTTP %s, 使用 PoW-only token",
            resp.status_code,
        )
        return json.dumps({
            "p": pow_token, "t": "", "c": "",
    ...
    try:
        ...
        result = {}
    server_token = str(result.get("token") or "")
    turnstile = result.get("turnstile") or {}
