...
`src/autoteam/manager.py` (`cmd_check`): 遍历 active 账号认证文件，调用 `src/autoteam/codex_auth.py` (`check_codex_quota`) 查询额度。额度不足时会写入 `last_quota`、`quota_exhausted_at`、`quota_resets_at`，并把账号标记为 `exhausted`。
