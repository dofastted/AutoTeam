...
- Auth 文件盘点由 `_AUTHS_FILE_PATTERN`、`_AUTHS_KNOWN_SUBDIRS`、`_AUTHS_PLAN_TYPES`、`_parse_auth_filename`、`_scan_auths_files`、`_bucket_primary_category`、`_bucket_to_record` 负责。已知 bucket 是 `active`、`sold`、`tradable`、`unusable`、`archive`。文件名支持 `codex-{email}-{plan_type}-{hash}-{oauth|session}.json`。
- `_bucket_primary_category(categories)` 决定同一邮箱跨目录时的主分类，优先级是 `sold` > `tradable` > `unusable` > `active` > `archive`；空集合返回 `unknown`。因此根目录 active 文件不会覆盖 `sold/`、`tradable/`、`unusable/` 分类目录，但会覆盖 `archive/` 备份目录。

账号管理相关接口：

- `GET /api/auths/stats`: 返回 auth 文件总数、OAuth 文件数、session 文件数、五类文件数、按邮箱主分类计数和唯一邮箱数。`*_files` 是文件口径，`accounts_*` 是账号口径。
- `GET /api/auths/accounts`: 返回按邮箱聚合的 auth 文件列表，支持 category、plan_type、邮箱搜索、OAuth/session 三态筛选、排序和分页。默认隐藏 `archive`，但显示非 Team plan。返回项包含 `plan_type`、`is_team_plan`、`is_limited_view`。
- `POST /api/accounts/bulk-action`: 统一处理账号批量动作。`confirm=false` 只预览；`confirm=true` 执行。支持 `mark-invalid`、`sell`、`delete`、`remove-team`、`cancel-invite`。主号和非 Team auth 记录会逐项跳过；`delete`、`remove-team`、`cancel-invite` 需要 `_playwright_lock`。
...
- `web/src/components/TeamMembers.vue`: 显示两张表，分别是 `type === "member"` 的真实成员和 `type === "invite"` 的邀请。列包含 email、类型、角色/状态、加入或邀请时间、本地账号状态和操作。member 可移出 Team，本地 active 可卖出，非主号本地账号可删除；invite 可取消邀请，也可删除匹配的本地账号。owner/main、sold、`sync_disabled=true` 行只显示状态。
...
- `web/src/components/AuthsTable.vue`: 扁平 auth 文件表，数据来自 `/api/auths/accounts`。支持 category、plan_type、OAuth 三态、Session 三态、排序、邮箱搜索、facets badge、20/50/100/200 分页、Team 行勾选和批量工具条。Auth 文件表不打开 `AccountDrawer`；非 Team 行只读，也不能进入危险批量选择。
...
`web/src/api.js` 现在包含 `getAuthsStats()`、`getAuthsAccounts(params)` 和 `bulkAccountAction(payload)`，分别对应 `/api/auths/stats`、`/api/auths/accounts` 和 `/api/accounts/bulk-action`。
