...
- Playwright 默认可见浏览器窗口。配置入口是 `src/autoteam/config.py` (`get_playwright_launch_options`)。
- API 模式下，同一时间只允许一个浏览器相关任务。相关控制在 `src/autoteam/api.py` (`_playwright_lock`, `_PlaywrightExecutor`)。
