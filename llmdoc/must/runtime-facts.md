...
- Playwright 默认无头启动。`PLAYWRIGHT_HEADLESS=false` 可改为可见窗口。配置入口是 `src/autoteam/config.py` (`get_playwright_launch_options`)。
- 同一进程内只允许一个 Chromium 流程。运行限制在 `src/autoteam/browser_runtime.py`，API 任务仍由 `src/autoteam/api.py` (`_playwright_lock`, `_PlaywrightExecutor`) 串行调度。
